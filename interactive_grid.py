...
def generate_squares(width, height, center, divider, number):
    ...
    for step in range(1, number+1):
        line_step_top = round(half_height_top/(divider**step))
        line_step_bot = round(half_height_bot/(divider**step))
    ...
    vertical_points = []
    for step in range(1, number+1):
        line_step_left = round(half_width_left/(divider**step))
        line_step_right = round(half_width_right/(divider**step))
...
def draw_squares(img, center, sequence, color_mode):
    '''this functions is derivative from draw_grid
    it draws squares only (including light square)'''
    if not sequence:
        return None
    ...
    for key, (p_start, p_stop) in enumerate(squares):
        ...
        if light_mode:
            ...
            img = cv2.rectangle(img, p_start, p_stop, squares_color, line_thickness)
        else:
            img = cv2.rectangle(img, p_start, p_stop, highlight_square_color, 1)
    return None


def update_points(img, points, center, show=True, freeze=False, gravity=0):
...
def main(height=1080, width=1920, points_number=180, fullscreen=True):
    ...
    quit_app = False
    while True:
        img[:, :] = 0       # it is faster, than creating new img everytime
        ...
        # ******* update & draw points *******
        points = update_points(img, points, center, show_points, freeze, gravity)
